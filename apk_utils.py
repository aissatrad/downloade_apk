...
def searching(package):
    res = requests.get(f"https://aapks.com/?s={package}")
    soup = BeautifulSoup(res.content, features="html.parser")
    apps = soup.findAll("div", class_="single-app")
    if apps:
        for apk in apps:
            pkg = apk.find("span", class_="app-details").text.split(" ")[-1].split(".apk")[0]
            if pkg == package:
                target = apk.find("div", class_="app-title").find("a")["href"]
                print(target.split("/")[-2])
                return apk.find("div", class_="app-title").find("a")["href"], target.split("/")[-2]
            else:
                return "", ""
    else:
        return "", ""
...
def links_versions(link, apk_name):
    versions = dict()
    if link != "" and apk_name != "":
        res = requests.get(f"{link}version/")

        soup = BeautifulSoup(res.content, features="html.parser")
        links = soup.findAll("div", class_="single-app")

        for apk_link in links:
            link = f'{apk_link.find("a")["href"]}dl/'
            js_res = requests.get(link)
            js_soup = BeautifulSoup(js_res.text, features="html.parser")
            js_scripts = js_soup.findAll("script")

            begin = 'http'
            end = '";'
            for js_link in js_scripts:
                st_js_link = str(js_link)
                if """var count = 20; // Timer""" in st_js_link and begin in st_js_link:
                    final_apk_link = st_js_link[st_js_link.index(begin):st_js_link.index(end)]
                    name = final_apk_link.split(r"/")[-1]
                    name = f"{apk_name} {name.split('-')[-3]}"
                    versions[name] = final_apk_link
